...
4. **Pull specialist inputs — confirm-then-dispatch pattern**
   ...
   - **4d. If user answers `y`:** dispatch each missing specialist in this order, ingesting each primary deliverable as it completes:
     ...
     - Each specialist runs its own `DATA_getSubscription` preflight and surfaces cost before proceeding (their existing behaviour — `seo-plan` does not bypass it). If any specialist aborts on a credit-balance check, surface that abort to the user and let them decide whether to top up or skip.
